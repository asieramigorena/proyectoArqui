...
def juego():
    ...
    while otra_vez:
        ...
        t_seleccion = threading.Thread(target=seleccion, args=(button,), daemon=True)
        t_comprobacion = threading.Thread(target=comprobacion, args=combinaciones, daemon=True)
        t_logica_led = threading.Thread(target=logica_led, args=(ledr, ledv), daemon=True)
